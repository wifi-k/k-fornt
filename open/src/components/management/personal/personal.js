...
import apply from '../../Authentication/apply'
...
export default {
    ...
    methods:{
      showApikeyicon(){
          self.showApikeyFlag = !self.showApikeyFlag
          // eye-slash
          if(self.showApikeyFlag){
              self.$refs['eye'].className='fa fa-eye-slash showapikey'
          }
          if(!self.showApikeyFlag){
            self.$refs['eye'].className='fa fa-eye showapikey'
        }
        },
        selfStatus(statusText){
        ...
        resetApikey(){
          self.$confirm('你确定重置apikey吗?', '确认重置', {
              ...
              Login.apikeyReset().then((p)=>{
                if(p.data.code==0){
                  ...
                  self.data.Apikey = infoData.apikey;
                  self.$notify({
                    title:'成功',
                    message:"重置成功",
                    type:'success',
