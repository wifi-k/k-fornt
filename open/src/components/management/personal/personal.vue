<template>
  <el-row class="wrapper">
      <el-col :span="12" class="wrapperleft">
        <h4 style="color:rgb(94,97,102);font-size:15px;">个人信息</h4>
        <el-form  label-width="100px" :model="data">
          ...
          <el-form-item label="Apikey：" v-if="data.Apikey" prop="Apikey">
            <span  class="lableData">
              <span v-show='showApikeyFlag'>{{data.Apikey}} </span>
              <span v-show='!showApikeyFlag'>******</span>
              </span>
              <a v-show='statusText=="审核通过"' @click='resetApikey'>重置</a>
              <span> <i class="fa fa-eye showapikey"  @click="showApikeyicon" ref="eye"></i></span>
        </el-form-item>  
        <el-form-item label="Apikey：" v-else  prop="Apikey">
            <span style="font-size:14px;color:#999999;">实名认证通过后可获取Apikey</span>
        </el-form-item>          
        </el-form>  
     ...
     <el-col :span="12" class="wrapper-right">
       <apply v-on:selfStatus='selfStatus'></apply>
     </el-col>
  </el-row>
</template>
